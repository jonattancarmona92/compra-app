...
class BluetoothPrintPlus {
  ...
  /// Consulta el estado actual del adaptador Bluetooth (encendido o
  /// apagado) sin iniciar escaneos. Actualiza también el stream [blueState]
  /// y devuelve el estado real consultado.
  static Future<BlueState> consultarEstadoBluetooth() async {
    _initFlutterBluePlus();
    try {
      final result = await _methodChannel.invokeMethod('state');
      _stateNow = result;
      if (result == 0) {
      ...
      // Si la consulta falla (p. ej. sin permisos), se conserva el último
      // estado conocido para no romper la interfaz.
    }
    return _blueState.latestValue;
  }
  ...
  /// teléfono. A diferencia del escaneo, estos no requieren descubrimiento
  /// y pueden conectarse de inmediato. Retorna una lista vacía si no hay
  /// vinculados o si el adaptador no está disponible.
  static Future<List<BluetoothDevice>> getBondedDevices() async {
    _initFlutterBluePlus();
    try {
      final raw = await _methodChannel.invokeMethod<List<dynamic>>(
          'bondedDevices');
      final lista = <BluetoothDevice>[];
      if (raw != null) {
        for (final item in raw) {
          final mapa = Map<String, dynamic>.from(item as Map);
          // `type` puede faltar en algunos teléfonos; el nativo siempre lo
          // envía, pero se defiende por si un parche lo omite.
          mapa['type'] = mapa['type'] ?? 0;
          lista.add(BluetoothDevice.fromJson(mapa));
        }
      }
      return lista;
    } catch (_) {
  ...
  /// Intenta encender (`true`) o apagar (`false`) el adaptador Bluetooth.
  /// Devuelve si la operación pudo completarse. En Android 12+ el sistema
  /// suele bloqueársela a apps de terceros (devuelve `false`); en ese caso
  /// use [openBluetoothSettings] para llevar al usuario a los ajustes.
  static Future<bool> setBluetoothEnabled(bool enable) async {
    _initFlutterBluePlus();
    try {
      final ok = await _methodChannel
          .invokeMethod<bool>('setBluetoothEnabled', {'enable': enable});
      return ok ?? false;
    } catch (_) {
      return false;
    }
  ...
  static Future<bool> removeBondedDevice(String address) async {
    _initFlutterBluePlus();
    try {
      final ok = await _methodChannel.invokeMethod<bool>(
          'removeBondedDevice', {'address': address});
      return ok ?? false;
    } catch (_) {
      return false;
    }
